 ...
 */
@Deprecated
public class TemporaryPlayerNameTakeover {
	...
	public static ArrayList<Location> getCurrentPath() {
		return TemporaryPlayerNameTakeover.currentPath;
	}
	...
	public static ArrayList<PreciseLocation> getCurrentAbovePath() {
		return TemporaryPlayerNameTakeover.abovePath;
	}
	...
	public static AStar makePath(Location l) {
		try {
			...
			if(path.getPathingResult() == PathingResult.SUCCESS) {
				...
				TemporaryPlayerNameTakeover.currentPath = theRealPath;
				TemporaryPlayerNameTakeover.pathStartLocation = a;
				return path;
			} else {
	...
	public static void clearPathData() {
		TemporaryPlayerNameTakeover.currentPath = null;
		TemporaryPlayerNameTakeover.abovePath = null;
		TemporaryPlayerNameTakeover.pathStartLocation = null;
	}
	
	private static Location walkHelper() {
		boolean inLiquid = getPlayer().isInWater() || getPlayer().handleLavaMovement();
		double vectorY = (vec3Helper(TemporaryPlayerNameTakeover.pathStartLocation)[1] + 1) - getLocation("").getY();
	...
	public static void faceLocation(PreciseLocation location) {
		...
		double degree = BrennyAngle.getAngle(new BrennyPoint(TemporaryPlayerNameTakeover.getPreciseLocation("").getX(), TemporaryPlayerNameTakeover.getPreciseLocation("").getZ()), new BrennyPoint(vectorX, vectorZ));
		while(degree < 0.0D)
			degree += 360.0D;
		rotateHead(degree);
		degree = BrennyHelpful.getAngle(Math.sqrt((vectorX * vectorX) + (vectorZ * vectorZ)), vectorY - TemporaryPlayerNameTakeover.getPreciseLocation(EYE).getY());
		pitchHead(degree);
	}
	...
	public static void swingItem(boolean toggle) {
		TemporaryPlayerNameTakeover.isSwingingItem = toggle;
	}
	...
	public static boolean isSwingingItem() {
		return TemporaryPlayerNameTakeover.isSwingingItem;
	}
	...
	public static void interact() {
		TemporaryPlayerNameTakeover.isInteracting = true;
	}
	...
	public static boolean isInteracting() {
		boolean tmp = TemporaryPlayerNameTakeover.isInteracting;
		TemporaryPlayerNameTakeover.isInteracting = false;
		return tmp;
	}
	...
	public static void useItem(boolean toggle) {
		TemporaryPlayerNameTakeover.isUsingItem = toggle;
	}
	...
	public static boolean isUsingItem() {
		return TemporaryPlayerNameTakeover.isUsingItem;
	}
	...
	public static void resetAttributes() {
		for(int i = 100; i < 104; i++)
			move(i, false);
		TemporaryPlayerNameTakeover.useItem(false);
		TemporaryPlayerNameTakeover.isInteracting = false;
		TemporaryPlayerNameTakeover.swingItem(false);
		TemporaryPlayerNameTakeover.preciseSpeed(0.0F, 0.0F);
		TemporaryPlayerNameTakeover.sprint(false);
		TemporaryPlayerNameTakeover.sneak(false);
		TemporaryPlayerNameTakeover.clearPathData();
	}
}